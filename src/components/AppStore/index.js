...
class AppStore extends Component{
  ...
  getFilteredApps = () => {
    ...
    const filteredApps = appsList.filter(eachApp => eachApp.category === activeTabId)
    return filteredApps
  }


  onChangeAppCategory = (tabId) => {
    const {activeTabId} = this.state
  ...
  render(){
    ...
    const searchResults = filteredApps.filter(eachApp => eachApp.appName.toLocaleLowerCase().includes(searchInput.toLocaleLowerCase()))
      ...
      <div className="bg-container">
        <div className="content-container">
          ...
          <div className="search-bar-container">
            <input value={searchInput} onChange={this.updateSearchResults} className="search-bar" type="search" />
            <img className="search-icon" alt="search icon" src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"/>
          </div>

          <ul className="tabs-list">
            {tabsList.map(eachTab => <TabItem isActive={activeTabId === eachTab.tabId} changeAppCategory={this.onChangeAppCategory} key={eachTab.tabId} tabDetails={eachTab} />)}
          </ul>

          <ul className="apps-list">
            {searchResults.map(eachApp => <AppItem key={eachApp.appId} alt={eachApp.appName} appDetails={eachApp}/>)}
          </ul>
